...
def show(request):
    ...
    question = category1[0].question.all().order_by('id')
    # print(question[0])
    question_object_list = list(question)
    # print(list(question))
    choice = question[0].choices.all()
    choice_set = []
    ...
    #     print (choice.choice)
    query_set = {"question": question[0].text, "choice": choice_set}
...
def register(request):
    ...
    if request.method == "POST":
        ...
        if form.is_valid():
            ...
            student_no = form.cleaned_data['StudentNo']
            ...
            data = Student.objects.create(name=name, student_no=student_no,
...
def instruction(request):
    # nothing to do here
    return render(request, "Exam_portal/instruction.html")
